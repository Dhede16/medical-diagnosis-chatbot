"""
widgets/chat_panel.py - Panel chat utama, ChatGPT-style.
- State "welcome": layar tengah dengan judul + suggestion chips
- State "chat": scrollable bubble area
"""
...
class ChatPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # ── Chat scroll (disembunyikan awalnya) ───────────────────────────
        ...
        # Belum di-grid sampai ada pesan
    ...
    def _build_input_bar(self):
        """Input bar mirip ChatGPT: rounded, full-width, tombol di dalam."""
        bar = ctk.CTkFrame(
            self,
            fg_color="transparent",
            corner_radius=0,
        )
        bar.grid(row=1, column=0, sticky="ew", padx=0, pady=(0, 16))
        bar.grid_columnconfigure(0, weight=1)

        # Container tengah dengan max-width
        ...
        self._is_ph = True
        self._set_ph()
        self._input_box.bind("<FocusIn>",  self._clear_ph)
        self._input_box.bind("<FocusOut>", self._restore_ph)

        # Send button (panah ke atas)
    ...
    def _set_ph(self):
        ...
        self._input_box.insert("1.0", self._ph)
    ...
    def _restore_ph(self, _=None):
        if not self._input_box.get("1.0", "end").strip():
            self._set_ph()
    ...
    def _suggestion_clicked(self, prompt: str):
        self._clear_ph()
        self._input_box.delete("1.0", "end")
        self._input_box.insert("1.0", prompt)
        self._input_box.configure(text_color=(TEXT_PRIMARY, TEXT_PRIMARY))
        self._is_ph = False
        self._do_send()
